...
def insertBalancesheetDataInDb():
    index = 0
    collection = db['balanceSheet_data']
    for symbol in df['Symbol']:
        if pd.notna(symbol) and symbol.strip():
            try:
                formattedData = {}
                balanceSheet = getUserRequiredData('balanceSheet', symbol)

                for obj in balanceSheet:
                    newObj = {}
                    for keys in obj:
                        if keys != 'index':
                            newObj[convert_epoch_to_iso8601(int(keys))] = obj[keys]
                    formattedData[obj['index']] = newObj
                
                formattedData['symbol'] =  f'{symbol}.NS'
                # print(formattedData)
                # Update database here using info dictionary
                collection.insert_one(formattedData)  # Example MongoDB insert operation
