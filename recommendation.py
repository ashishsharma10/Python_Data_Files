...
def insertRecommendation():
    collection = db['share_recommendation']
    index = 0
    for symbol in df['Symbol']:
        if pd.notna(symbol) and symbol.strip():
            try:
                shares_recommendation = getUserRequiredData('recommendation', symbol)

                # Update database here using info dictionary
                collection.insert_one({'symbol': f'{symbol}.NS', 'recommendations': shares_recommendation})  # Example MongoDB insert operation
